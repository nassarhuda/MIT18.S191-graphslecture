...
c.Hide_output = true
c.Store_csv = true
c.Output = "../Julia-twitter-graph/julialang_tweets.csv"
c.Search = "#julialang"
c.Min_likes = 5
curtweets = twint.run.Search(c)

# second, get all the usernames that tweeted those tweets
julia_tweets = CSV.read("../Julia-twitter-graph/julialang_tweets.csv")
...
for (curi,cur_username) in enumerate(unique_usernames)
    ...
    h.Hide_output = true
    h.Store_csv = true
    h.Output = join(["../Julia-twitter-graph/following_",cur_username,".csv"])
...
for i = 1:n
    ...
    try
        curfollowing = CSV.read(join(["../Julia-twitter-graph/following_",curusername,".csv"]))[:username]
